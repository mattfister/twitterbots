...
var numAdded = 0; // How many tweets we've added
...
var passesFilter = function(tweet) {
    if (tweet.indexOf("trending") > -1) {
	return false;
    }
    if (tweet.indexOf("1.") > -1 && tweet.indexOf("2.") > -1) {
	return false;
    }
...
function addTweet(tweet) {
	...
	words[i] = words[i].replace("\"", ""); // Strip out quotes around words - it's just confusing
	words[i] = words[i].replace("&amp;", "&"); // Replace ampersands with ampersands
	words[i] = words[i].replace("&gt;", ">"); // Replace greater thans with >
	words[i] = words[i].replace("&lt;", "<"); // Replace less thans with <
	words[i] = words[i].replace("&#39;", "'"); // Replace quotes with '
    }
...
// and no bigger than 140 characters. Generate 10 of these and take the best defined as
// the sum of the number of words that follow each word
var markovTweet = function(minLength) {
	...
	while (goodTweet == false ) {
		...
		if (curRank > maxRank) {
		    maxRank = curRank;
		    bestTweet = tweet;
		    console.log(i + "\tRank: " + maxRank + " Tweet: " + bestTweet);
		}
	    }
	}
    }
...
// This keeps track of how our streaming is doing
...
function monitorStreaming() {
	...
	console.log("Error in monitor streaming() " + e);
    }
}


connections = [" do you know ", " you know ", " what's up with ", " who is ", " what do you think about ", " who's ", " I'd like to introduce you to "] 
...
function connectorStream() {
	...
	T.get('trends/place', {id: '23424977'}, function(err, reply) {
		...
		try {
		    ...
		    console.log("Error while streaming; tweet: " + tweet + " e: " + e);
		}
	    });
	});
    } catch(e) {
	console.log("Error in connectorStream, e: " + e);
    }		   
}
...
repliedToUsers = {};
replyMessages = [" you both seem p cool", " thought you were both interesting", " :)", " you're cool", " you both seem chill", " lol", " my bad"]
...
function handleMentions() {
	...
	T.get('statuses/mentions_timeline', {}, function(e,r) {
		...
		console.log("Error in getting mentions callback, e: " + e);
	    }
	});
    } catch(e) {
	console.log("Error in handleMentions, e: " + e);
    }
}


// Every 15 seconds see how we're doing
setInterval(monitorStreaming, 1000 * 15); 

// Every 1 minute  try to start streaming
setInterval(connectorStream, 60000*1);

// Wait 2 seconds after startup and start posting
setTimeout(connectorStream, 2000);

// Every 10 minutes see if we have new mentions
setInterval(handleMentions, 60000 * 10);

// Get our first mentions 5 seconds after we start
setTimeout(handleMentions, 5000);

// Do a quick first pass on mentions just to test
setTimeout(handleMentions, 60000 * 1);
